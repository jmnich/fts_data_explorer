...
def main(argv=None):
    ...
    print()
    print("=" * 60)
    print("Step 8 — Multi-workspace .h5 suite (M2.4)")
    print("=" * 60)
    mwRt = None
    for cand in BIN_DIR_CANDIDATES:
        cand_rt = cand / "fts_multi_workspace_roundtrip"
        if cand_rt.exists():
            mwRt = cand_rt
            break
    if mwRt is None:
        print("fts_multi_workspace_roundtrip binary not found — skipping step 8")
    else:
