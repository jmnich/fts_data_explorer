...
// Multi-workspace store (`.h5`) — the embedded multi-workspace format (schema v2). Layout:
//
...
//
// The multi-workspace file has NO root @format attribute: H5Store::load must keep
// rejecting it (missing @format) — the archive.json sniff in every open path
// routes multi-workspace files to multiWorkspaceLoad before that can happen.
//
...
// and group are untouched — multiWorkspaceSaveSource preserves this name on save-back.
...
// multi-workspace .h5 can restore: embedded workspaces + persisted experiments only.
std::vector<std::string> persistableTabOrder(const AppState& s);

// ── Phase 4: experiments (schema v2 layout) ──
//
...
// shared by the fts_multi_workspace_roundtrip CLI and the AppState-level wrappers
...
// multiWorkspaceSaveExperiments saves every dirty instance (exit Save All). Loading
...
bool multiWorkspaceLoadExperiments(AppState& s, const std::string& path, std::string& err);
...
// All mutations are atomic (atomicMutate temp-copy + rename, store
