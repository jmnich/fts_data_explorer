...
public:
    ...
    // ── Difference (Comparator + Absorbance) ──
    ...
    int  differenceRefIdx = 0;        // index into the visible-curve list
    int  differenceSubIdx = 1;        // index into the visible-curve list
    int  differenceMode = 0;          // 0 signed, 1 absolute
    int  differenceStatsRegion = 0;   // 0 viewbox (current X window), 1 all
    // Live difference result (full-res, post-mode, recomputed every render
...
private:
    ...
    double differencePeakPeak = 0.0, differenceStddev = 0.0;
    bool   differenceStatsValid = false;
    ...
    bool differenceWasShown_ = false;
    // Curve labels cached at the end of renderViewWindow (the curve vector is
    // local to it) so renderDifferenceWindow — which runs later in the SAME
    // frame — can populate the reference/subtracted comboboxes.
    std::vector<std::string> differenceCurveLabels_;
    ...
    // Real-time difference compute from the visible curves (called inside
    // renderPlot, after the dB params). Synchronous; clears differenceX/Y and
    // invalidates stats on every refusing condition.
    void computeDifference(const std::vector<ComparatorCurve>& curves,
                         bool dBNormalize, double dBRefMax);
    // Min/max/peak-peak/stddev (sample N-1 via RunningStats) over the difference
    // curve; region = viewbox X window or all. Sets differenceStatsValid.
    void computeDifferenceStats();
    // Docked "Difference" panel: enable, reference/subtracted comboboxes,
    // signed/absolute, stats region + statistics display.
    void renderDifferenceWindow();
