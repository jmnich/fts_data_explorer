...
void test12_experimentPersistence() {
    ...
    cmp->tabHidden = true;
    // Difference (both types): config round-trips through config.json and the
    // live vectors ride the results datasets (full-res, post-mode). Loaded
    // data is a seed — the first render recomputes live.
    cmp->differenceEnabled = true;
    cmp->differenceRefIdx = 0;
    cmp->differenceSubIdx = 1;
    cmp->differenceMode = 1;          // absolute
    cmp->differenceStatsRegion = 1;   // all
    cmp->differenceX = {1500.0, 1750.0, 2000.0};
    cmp->differenceY = {0.1, 0.0, 0.25};
    ...
    // Difference round-trips (config + result datasets).
    CHECK(c2->differenceEnabled == true);
    CHECK(c2->differenceRefIdx == 0);
    CHECK(c2->differenceSubIdx == 1);
    CHECK(c2->differenceMode == 1);
    CHECK(c2->differenceStatsRegion == 1);
    checkVecEq(c2->differenceX, cmp->differenceX, "restored difference_x");
    checkVecEq(c2->differenceY, cmp->differenceY, "restored difference_y");
    CHECK(a2b->differenceEnabled == false);   // default: disabled
    ...
    CHECK(multiWorkspaceSaveExperiment(s4, *c2, multiWorkspacePath, err));
    {
        ...
        CHECK(cfg2["differenceEnabled"] == true);
        CHECK(res2.count("difference_x") == 1);
        checkVecEq(res2["difference_x"], c2->differenceX, "persisted difference_x");
        checkVecEq(res2["difference_y"], c2->differenceY, "persisted difference_y");
    }
