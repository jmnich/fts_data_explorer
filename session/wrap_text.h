...
// per-char width callback, at most `maxLines` lines. When clamped, the last
// line is trimmed and suffixed with "…" (ellipsisWidth must include its own
// width). No ImGui dependency — the GUI feeds CalcTextSize in via the
// callback (see session_tab.cpp's wrapToLines). Tested by
// playground/tests/wrap_text/test_wrap.cpp.
std::vector<std::string> wrapToLinesCore(const std::string& text, float maxWidth,
    ...
    while (i < text.size() && static_cast<int>(lines.size()) < maxLines) {
        ...
        if (j >= text.size()) {
            ...
            // the overflow char is not in the line — resume AT it.
            i = atSpace ? breakAt + 1 : (j > i ? j : i + 1);
        }
