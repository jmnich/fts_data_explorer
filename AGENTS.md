...
**Built-in converters** (`converters/`, dev/test source — the public repo is the delivery channel): `wust_mini_fts.py` (raw_data/*.csv, dual IFG), `arcoptix_igms.py` (OPD vs IGM .txt → `igm_corrected_x/`), `arcoptix_spectra.py` (spectra .txt → `spectra/` originals).
